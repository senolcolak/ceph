...
public:
  ...
  // Restricts delegated access to specific service/method/path combinations.
  class AccessRule {
  public:
  ...
  User user;
  std::list<Role> roles;
  std::vector<AccessRule> access_rules; // Application credential access rules
...
public:
  ...
  const std::vector<AccessRule>&
  get_access_rules() const
  {
    return access_rules;
  }

  bool
  has_access_rules() const
  {
    return !access_rules.empty();
  }
