...
// Service type constant for Swift/Object Store access rules
static constexpr const char* const SWIFT_SERVICE_TYPE = "object-store";

// Match path against pattern with OpenStack-style glob support.
// Patterns support:
...
{
  // Convert OpenStack '**' pattern to fnmatch-compatible pattern.
  // '**' should match anything including '/', while '*' matches except '/'.
  //
  // Strategy: Replace '**' with a sentinel, use fnmatch without FNM_PATHNAME
  // for '**' portions, but that's complex. Instead, we do manual matching.

  size_t pi = 0; // pattern index
  size_t si = 0; // path (string) index
  ...
  while (si < path.size()) {
    ...
    // If we have a '**' to backtrack to, consume one char from path
    ...
    // If we have a '*' to backtrack to, consume one non-'/' char from path
  ...
  // Consume trailing '*' and '**' in pattern
...
// Check if request matches any access rule in the token.
// Returns true if access is granted, false if denied.
// If rules list is empty, returns true (no restrictions).
static bool
check_access_rules(
...
{
  // No rules means no restrictions (standard token without app credentials)
  if (rules.empty()) {
    return true;
  }

  for (const auto& rule : rules) {
    // Service must match object-store
    if (rule.service != SWIFT_SERVICE_TYPE) {
      continue;
    }

    // Method must match (case-sensitive per OpenStack spec)
    if (rule.method != method) {
      continue;
    }

    // Path matching with OpenStack-style glob support (including **)
    if (path_matches_pattern(rule.path, path)) {
      ldpp_dout(dpp, 10) << "access rule matched: service=" << rule.service
...
admin_token_retry:
  ...
  // Signal to Keystone that we support Application Credential Access Rules.
  // This tells Keystone to include access_rules in the token response.
  ...
  for (const auto& role : roles.plain) {
    if (t->has_role(role) == true) {
      ...
      if (!check_access_rules(
              dpp, t->get_access_rules(), s->info.method, s->decoded_uri)) {
