...
TEST(RGWSecureEndpoint, rejectsSpecialAddresses)
{
  for (const auto* value : {"0.0.0.1", "127.0.0.1", "169.254.169.254",
                            ...
                            "100.64.0.1", "192.0.2.1",
                            "192.31.196.1", "192.52.193.1",
                            "192.175.48.1", "198.18.0.1",
  ...
  boost::system::error_code ec;
  for (const auto* value : {"3ffe:ffff::1", "3fff:1000::1"}) {
    const auto adjacent = boost::asio::ip::make_address(value, ec);
    ASSERT_FALSE(ec);
    EXPECT_FALSE(policy::is_prohibited_address(adjacent)) << value;
  }
}
